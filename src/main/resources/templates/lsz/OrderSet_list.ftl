    ...
    <div class="layui-colla-item">
        <p class="layui-colla-title">
				<span id="menutreeclickdetailid"
                      class="layui-badge-rim layui-bg-green">单据配置</span>列表
        </p>
...
<script type="text/html" id="opeartion">
 <input type="hidden"  value="{{d.id}}">
	...
	<script type="text/javascript"
		src="${ctx}/js/lsz/OrderSet_m.js"></script>
	<script>
		layui.use('table', function() {
			...
			table.render({
				elem : '#ztable',
				url : 'orderSetList.php',
				cols : [ [ {
					type : 'checkbox'
				} , 

{field : 'name', title : '名称' },
{field : 'leftStr', title : '前戳' },
{field : 'title', title : '标题' },
{field : 'orderfile', title : '单据文件名' },
{templet : '<div>{{ shifou(d.adds ) }}</div>', title : '增加库存' },
